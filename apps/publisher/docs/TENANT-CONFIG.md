...
| `pageToc` | bool/object | `false` | "On this page" heading nav + scroll-spy, generated client-side from each page's `h2`/`h3`. `true` ⇒ right rail; `{ placement: "rail" \| "left" \| "top" \| "off", minHeadings: N }` configures placement and the heading-count threshold (default 3). **`rail`** — a pinnable right-gutter panel (unpin to collapse it to a prev/next bar that reveals on hover/tap). **`left`** — mirrors the main nav: a persistent list stacked under the site navigation in the sidebar, leaving the content full-width. **`top`** — a bordered block above the content. Accessible (`<nav aria-label>`, real links, `aria-current`); content is complete with JS off (the nav is an enhancement). |
